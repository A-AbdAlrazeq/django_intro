...
def process_money(request):
    ...
    if request.session['move'] > 14 and not request.session['end_game']:
        if request.session['gold'] >= 100:
            request.session['activities'].insert(
                1, f"You won! ({now}) - Reset to play again!")
        else:
            request.session['activities'].insert(
                1, f"You lost.. ({now}) - Reset to play again..")
    ...
    if request.POST['action'] == "casino":
        ...
        if random.randint(1, 4) % 2 == 1:
            ...
            request.session['activities'].insert(
                1, f"Entered a casino and lost {gold} golds.. Ouch.. ({now})")
        else:
            request.session['gold'] += gold
            request.session['activities'].insert(
                1, f"Entered a casino and earned {gold} golds! Yeah! ({now})")
    else:
        ...
        request.session['gold'] += gold
        request.session['activities'].insert(
            1, f"Earned {gold} golds from the {request.POST['action']}! ({now})")
